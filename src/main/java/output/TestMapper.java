...
public class TestMapper {
	public TargetClass map(SourceClass source) {
		...
		target.setListC(mapListCToListC(source.getListC()));
		target.setTargetListB(mapSourceListBToTargetListB(source.getSourceListB()));
		return target;
	}
	...
	private List<TargetType> mapSourceListBToTargetListB(List<SourceType> list) {
		List<TargetType> res = new ArrayList<>();
		for (SourceType src : list) {
			TargetType tgt = new TargetType();
			tgt.setTypeA(src.getTypeA());
